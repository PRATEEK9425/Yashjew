...
const SilverFinalorder = () => {
    ...
    useEffect(()=>{
      ...
      const Handlechange=(e)=>{
        ...
        console.log(payload);
        axios.post(`/website/orderlist/create-webgoldorder`,payload)
        .then((res)=>{
...
const { GoldImg,Title,Category,Model_No,Availability_Type,Gold_Karat,Quantity,Name,Email,Mobile_Nmb,Address,PinCode} =formdata
...

{
      ...
      <div  className='grid grid-cols-2 gap-2'  >
        {/* div1  */}
      <div  >
        <img className='m-auto rounded-2xl hover:w-[150%] hover-h-[150%] '  src={`http://localhost:3500/${formdata.GoldImg}`} alt="" />
...
<div>Availability   <span className='text-green-800' >{formdata.Availability_Type} </span> </div>
      ...
      <h1 className='text-3xl  font-serif text-transparent    bg-clip-text bg-gradient-to-r from-yellow-500 to-pink-700   '  >Purity - {formdata.Gold_Karat}</h1>
...
<input name="Gold_Karat" defaultValue={Gold_Karat} onChange={Handlechange}  required  placeholder=' Title 🤴'  className={styles.textbox} type="string"  />
...
export default SilverFinalorder