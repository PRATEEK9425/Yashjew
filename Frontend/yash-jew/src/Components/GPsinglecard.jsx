...
if(data.length==0){
  ...
  return (
    ...
    <div   >
      <h1 className='text-5xl mt-14 font-serif text-transparent    bg-clip-text bg-gradient-to-r from-yellow-500 to-pink-700   '  >{data.Title}</h1>
      <span className='text-3xl font-mono text-red-400  ' >Gender -  {data.Category=="Male" ? <span> {data.Category}  🦸‍♂️ </span>  : <span>{data.Category}  🦸‍♀️</span>} </span>
...
<div>Availability   <span className='text-green-800' >{data.Availability_Type} </span> </div>
      ...
      <button onClick={()=>navigate(`/GP-Web-place-order/${ornamentstype}/${data._id}`)}   className=" font-serif mt-4  border-4 hover:border-cyan-700 md:hover:text-xl  md:hover:font-bold  text-xl text-white rounded-xl  h-14 w-[200px]  md:h-16 md:w-[200px] bg-gradient-to-r from-orange-600 via-amber-400 to-red-400 ..."   >ORDER NOW 💁</button>
    </div>
