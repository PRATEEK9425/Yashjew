...
const navigate = useNavigate()
  ...
  const Delfn=(id)=>{
   ...
   const Editfn =(id)=>{
navigate(`/admin_goldproductEdit/${id}/${cat}`)
   }
...
<img className='rounded-2xl h-[300px] w-full ' src={`https://yashjewbackend.onrender.com/${item.GoldImg}`} alt="" />
<h1>{item.Title}</h1>
<h1>{item.Category}</h1>
<h1>{item.Model_No}</h1>
<h1>{item.Availability_Type}</h1>
<h1>{item.Quantity}</h1>
<h1>{item.Gold_Karat}</h1>
