...
const  Resetcheck3 = () => {
  ...
  const validuser = useSelector(state=>state.EmailReducer)
 ...
 const {useremails} = useParams()
  ...
  axios.post(`${process.env.React_App_Resetcheck3_url1}`,payload)
  .then((res)=>{
...
if(ValidUser==="Email verified" && UserID.length>0){
  navigate(`/resetcheck4/${useremail}`)
}
...
const DeleteEntryfn=(id)=>{
 let UserId = id
 console.log(UserId);
axios.delete(`${process.env.React_App_Resetcheck3_url2}/${UserId}`)
.then((res)=>{
...
.catch((err)=>{
  ...
  return (
    <div>
      ...
      <span className=" h-full mt-10 mb-14 font-extrabold text-transparent text-2xl  md:text-3xl  lg:text-6xl bg-clip-text bg-gradient-to-r from-red-300 to-red-900" >
        Verify Your  Email.. 
      </span>
      <span className='text-2xl  md:text-3xl  lg:text-6xl'  >🚀</span>
        <div className=' w-[70%] m-auto mt-[50px]   grid grid-cols-1 md:grid-cols-2 h-[500px]  gap-4  mb-[300px] sm:mb-[500px]  md:mb-[100px] ' >
            {/* div 1 */}
        <div    >
...
{/* div 2 form */}
    ...
    <br/>
    <br/>
    <input onChange={HandleOnchange}   name="useremail" value={useremails} placeholder='    Email 📧'  className='rounded-md h-9 w-80 border  hover:border-blue-400 '   type="email" required  />
    <br/>
    <br/>
